...
# Trigger Zone — AI guide

An invisible named volume that knows who is standing in it. Other scripts react
to it BY NAME through the zone bus carried in this folder — nothing is wired,
...
start an ambience. One zone per place — though several areas may share a Name,
and the bus then treats them as one place.

## API

A ZONE'S ID IS THE ENTITY'S NAME, matched trimmed and case-insensitively — the
[Scene] block lists the zones the scene already has.
...
- playersInZone(name): every avatar currently inside, deduped across areas sharing the name.
...
- zoneOf(entity): the zone name of the entity a script is attached to ('' if unnamed).

Full signatures (and zoneNames()) are in the module header: scripts/runtime/zoneBus.ts
in this folder.

Params of the zone's own script — set them in the placePrefab request, never
tell the user to change them:
- who: 'this player' (default) | 'any player' — whose client the zone reacts to.
...
- exitDelay: seconds (default 0.3) a player may step back out before counting as
  ...
  and naming that limit "cooldown" next to the zone's settings reads as this one;
  name it for what it limits and say so in its doc comment.

## Occupancy beats edges

Most "when someone is in here" behaviour is occupancy, not events: isInZone() in
...
Use onZone(name, 'enter', …) only when the behaviour really is one-shot, 'exit'
...
An entity's Script component is a LIST — a zone carries its detector AND any
reactions. Decide by what the behaviour acts on:
...
- It changes ANOTHER object ("open the door", "turn on the lights") → attach to
  ...
  'Front Door Zone'. A reaction living elsewhere needs telling which zone; never
  hardcode the name where a param would do.
- It acts on the player, the UI, the sound, the score — nothing else involved
  ("play an emote", "show a message", "give points") → attach to THE ZONE ITSELF,
  with an attachScript request naming the zone, and give it NO zone param at all.
  ...
  which zone, and renaming the zone can't orphan it. Exactly this shape, params
  for the behaviour only:
      ...
      export class ZonePoints {
        ...
        update(dt: number) {}
        private award(address: string) { /* the behaviour goes here */ }
      }

- NEVER pick an unrelated entity just to have somewhere to put a script. No
  object involved → the zone is the answer.
- A zone named in the prompt ("…when they enter Front Hall") says WHICH zone to
  attach to — it is not a request for a zone param.

## Do / Don't

- DON'T subscribe triggerAreaEventsSystem.onTriggerEnter/Stay/Exit on a zone
  entity. The SDK keeps exactly ONE callback per (entity, event), so your
  subscription silently replaces the prefab's own detector and the zone stops
...
- The volume IS the entity's Transform scale in metres, so the scale gizmo is the
  ...
  place the zone NEAR the object at a sensible size and say in one line to drag
...
- Detection is client-side only: the headless server has no avatar colliders, so
  a zone never fires there. Fine for doors, lights, sound, ambience. When a zone
...
"Front Door Zone" (4×3×4 at the doorway, who: 'any player'), then a script on
