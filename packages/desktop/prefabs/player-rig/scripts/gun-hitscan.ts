...
//
// It reports; it never decides. The payload is the instance id of whatever was
// hit plus the damage this client believes it deals — the server's validator on
// the ledger below is what clamps the number, enforces the rate and subtracts
// the hit points. A client that reports faster or harder gets refused, not
// obeyed. And in a 'planned' spawn (zombies that only ever exist on clients) the
// server cannot check WHERE the shooter was, so proximity is not validated at
// all: say "server-validated damage", never "cheat-proof".
//
...
export class GunHitscan {
  ...
  constructor(
    ...
    /** Damage this client requests per hit; the server clamps it to its own table. */
  ...
  update(dt: number): void {
    ...
    this.cooldown = 1 / Math.max(0.1, this.shotsPerSecond)
    this.fire()
  }
    ...
    raycastSystem.registerLocalDirectionRaycast(
      ...
      (result) => {
        for (const hit of result.hits) {
          ...
          hits.report(HIT_KIND, { instanceId, amount: Math.max(1, Math.floor(this.shotDamage)) })
          return
        }
      }
    )
