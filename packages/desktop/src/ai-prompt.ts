...
- Say what a clone actually guarantees — it differs by the prefab doing the cloning, and that prefab's guide states it (a Spawner's copies exist only on the screen of the player whose trigger made them). Never "verified" or "cheat-proof".
...
Anything players share — scores, rounds, winners, what is open or locked — goes through the \`game\` module: import { game, onClick, childrenOf } from './runtime/game' (that path from a script in src/scripts/). One shared game runs on the server; each player's screen runs its own copy of every script. Which side a line runs on is decided by WHICH callback you put it in, never by a runtime check: there is no isServer(), and syncEntity, MessageBus and room.* never belong in a script.
- Runs on the server, for everyone: game.onStart(fn) — once when the game wakes; game.onMessage(name, (data, player) => reply); game.onPlayerJoin(fn) / game.onPlayerLeave(fn); game.onEnterArea(name, fn) / game.onExitArea(name, fn) — name is a Trigger Area entity's Name, and the game re-checks the player's real position before it counts; game.onRoundStart(fn); game.every(seconds, fn). Only inside these may you call game.setState(patch), game.newRound(), game.saved.get/set (survives restarts), game.playerData(player).get/set (one record per wallet) and game.positionOf(player) — elsewhere they throw.
- Runs on each player's screen: game.onStateChange(fn); game.layout(prefab, (rng, round) => positions) — the same layout everywhere with zero messages; onClick(entity, fn) for what THIS player clicks; childrenOf(entity) for the entities dragged under one. game.state, game.round and game.now() (the shared clock) read the same on both sides.
- game.send(name, data) is symmetric: a screen's send asks the game and resolves with the handler's return; the game's send tells every player — a moment, shown once, so lasting facts go in game.state. game.send/game.onMessage are the Multiplayer Server's room.send/room.onMessage with the envelope handled for you.
- The runtime also enforces: game.state.round is reserved (start rounds with game.newRound()); one handler per message name per side — a second script claiming the same name errors; a message that carries too much data or arrives too often is refused, so send ids and numbers, never blobs.
