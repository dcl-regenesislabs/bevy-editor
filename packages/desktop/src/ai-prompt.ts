...
- Runs on the server, inside the branch: game.onReady(fn) — once when the server wakes, and the only place game.saved has loaded (read in start() it gives nothing); game.onRequest(name, (data, player) => answer) — answers a message a player sent, and \`player\` is server-verified; game.broadcast(name, data?, player?) — one-way to everyone, or to one player; game.onPlayerJoin(fn) / game.onPlayerLeave(fn); game.onEnterArea(name, fn) — name is a Trigger Area entity's Name, and the server re-checks the player's real position before it counts; game.onRoundStart(fn); game.every(seconds, fn). Only on this side may you call game.setState(patch), game.newRound(), game.saved.get/set (survives restarts), game.playerData(player).get/set (one record per wallet) and game.positionOf(player).
...
- The runtime also enforces: game.state.round is reserved (start rounds with game.newRound()); one handler per message name — a second script claiming the same name errors; a message that carries too much data or arrives too often is refused, so send ids and numbers, never blobs.
