...
- { "type": "setParams", "to": "Crate Spawner", "params": { "spawn": "Crate" } } changes settings on a script ALREADY in the scene — target the entity the [Scene] block lists that script under, which for a prefab is often a child rather than its root.
- A setting that takes a prefab is set by NAME from the [Spawnable prefabs] block ("spawn": "Crate"), or a list of names for one that takes several ("arenas": ["Ruins", "Rooftop"]). An unknown or ambiguous name is skipped with a notice, never guessed.
...
- React through the zone bus: import { isInZone } from './runtime/zoneBus' (that path from a script in src/scripts).
