...
import { BUILD_TAB_EMPTY, GAME_TAB_EMPTY, GAME_TAB_ORDER } from './log-roles'
...
describe('the Game tab', () => {
  ...
  it('shows the shared copy’s lines, which arrive on the build stream', async () => {
    ...
    window.editorShell = shell(['✓ port 8004: server is up (3.1s)', '[game] round 2 started'])
  ...
  it('says how the two copies were ordered, but only when both printed', async () => {
    sceneLogs.mockResolvedValue(LOGS)
    window.editorShell = shell(['[game] round 2 started'])
    const drawer = mount(<LogsDrawer open initialTab="scene" onClose={() => {}} />)
    await drawer.settle()
    expect(drawer.text()).toContain(GAME_TAB_ORDER)
    drawer.unmount()

    sceneLogs.mockResolvedValue(LOGS)
    window.editorShell = shell([])
    const alone = mount(<LogsDrawer open initialTab="scene" onClose={() => {}} />)
    await alone.settle()
    expect(alone.text()).not.toContain(GAME_TAB_ORDER)
    alone.unmount()
  })
})
