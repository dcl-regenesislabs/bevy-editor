...
import { uiLoadLocalImages, uiImportImage, uiResolveImageUrl } from '../actions/assets'
import { Checkbox, Select, Toggle } from '../ds'
import {
...
function Sel<T extends string>(props: { value: T | undefined; onChange: (v: T | undefined) => void; options: readonly T[]; empty?: string }): JSX.Element {
  return (
    <Select
      density="compact"
      value={props.value ?? ''}
      options={[{ value: '', label: props.empty ?? '—' }, ...props.options.map((o) => ({ value: o, label: o }))]}
      onChange={(v) => props.onChange((v === '' ? undefined : v) as T | undefined)}
    />
  )
}
    ...
    <>
      ...
      <div style={{ width: 56, flex: 'none' }}>
        <Select
          ...
          onChange={(v) => props.onChange({ value: d?.value, unit: v as 'px' | '%' })}
        />
      </div>
    </>
  )
...
function Bool(props: { value?: boolean; onChange: (v: boolean) => void }): JSX.Element {
  return <Toggle size="sm" checked={props.value === true} onChange={props.onChange} />
}
...
export function BuilderInspector(): JSX.Element {
  ...
  if (!n) {
    return (
      <div className="eui-panel eui-uib-side-right">
  ...
  return (
    <div className="eui-group">
      <div style={{ marginTop: 6 }}>
        <Checkbox checked={on} onChange={(c) => updateNode(node.id, { uvs: c ? [0, 0, 0, 1, 1, 1, 1, 0] : undefined })}>
          Atlas UVs
        </Checkbox>
      </div>
      {on && node.uvs && (
