...
- **NEW PREFAB: Finish Line** (a Checkpoint/Finish zone variant). Intrinsic occasion: verified entry, once per player per round (needs `game.round.number`). Intrinsic action, an enum: *record best time* or *award N points*; writes a board key. This single prefab closes the first wall of **both** race and towerofmadness-lite entirely inside the doctrine — the two coverage rows that currently hide wall (c). It is the kit's honest answer to "the Finish zone does nothing."
