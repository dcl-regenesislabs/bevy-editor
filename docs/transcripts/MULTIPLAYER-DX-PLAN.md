...
## 16. Implementation review — 2026-08-08 (branch `feat/multiplayer-core`)

A six-agent adversarial review walked the Tower of Madness session against the shipped code and audited it for bugs, security holes and test quality. Full findings: `docs/MULTIPLAYER-IMPL-REVIEW.md`. It found two of §11's headline guarantees false in the implementation and several ways the walkthrough stopped working. Fixed on the branch (commits `ae62a75`, `2a00a26`):

- **Forged shared facts.** Per-entity guards only cover entities the run created; the SDK mints a fresh entity for an id it has never seen, and an unguarded component is accepted by default — so a modified client could publish a fact every screen would adopt. The server now arms the component-wide validator once, refusing anyone who is not the game, and revisions off the wire are read as data.
- **Whispers were broadcasts.** `{to}` rode in the payload while the packet went to everyone; it now goes to the transport, which is what enforces delivery.
- **One packet could poison a name.** An inbound ask claimed a name's direction before the handler lookup, so a forged packet ran screen code in the game and killed that name for the session. Unknown and screen-side names are refused without touching the registry; the game's own send decides direction and warns when a name is used both ways.
- Reading a departed player's data no longer throws (it aborted round-end tallies and wedged the round loop); the `round` key is reserved with a teaching error; an unserializable value costs one key instead of every frame; re-claiming a zone no longer buys another unverified grace window; zone claims are rate-limited like any other ask; a burst of asks costs one storage read; a failed boot still opens the gate; layout errors keep their registration and name the fix; `childrenOf` ships; the surface takes optional types.

**Still open, tracked, not blocking a first human review:** the green-context flag should be a depth counter (concurrent green spans can revoke each other's permission); `send` from a placed script's `start()` takes the blue path on the server because the role is unresolved at that moment; rpc retries have no idempotency cache, so a dropped reply can double-credit an economy handler; wire replies still carry internal error text.

**The gating question before PR 8 (`V1`):** does `playerPosition` return anything on a real headless server? `playerPositions.ts` says it returns `[]` there, while the shipped `zone-authority` prefab re-derives positions from comms and works in production. Every position-dependent verb — zone re-verification, `positionOf`, and therefore the tower's finish check — rests on the answer. It must be probed on a real headless server before the template flip, and if the answer is `[]` the design needs a different answer, not a patch.
