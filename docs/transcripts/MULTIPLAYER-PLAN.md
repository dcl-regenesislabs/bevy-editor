...
> **Status (2026-08-06):** the built-in **Spawner** prefab landed ahead of M6 (see `docs/PREFABS.md`) — the generic "make a copy appear while the game runs" primitive the deferred NPC/Mob base and Score/XP prefabs would each have re-invented. It shipped 0.2.x as a server-decided design (a `spawnBus` rpc with server-minted ids and persisted alive-set) and was **rebuilt client-side in 0.3.0**: the trigger fires on the player's game and the copy is built right there from the `'seeded'` pool — per-spot cap, lifetime and deterministic scatter kept; nothing crosses the wire, nothing is stored between plays (the persisted alive-set resurrected copies on every preview boot), and the prefab no longer carries `requiresSdk`. `requestSpawn`/`retireSpawned` live in the tiny `spawnPoints.ts` registry. Three scene checks (`mixed-pool-authority`, `spawner-click-no-collider`, `spawner-nested-spawn`) and a validate probe (`probe-spawner.mjs`) guard it. It is also the first prefab with a **right-click gesture that configures it for you** — which is why a parser fix (`TSAsExpression`) and the nested-instance exclusion in `instanceDrift` rode along. Server-agreed spawns (a contested pickup every player must see) are a future authoritative pattern on the kit prefabs, not this one.
