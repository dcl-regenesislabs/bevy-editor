...
| **Prefabs** | Select what you built and use **Create prefab…** (right-click in the hierarchy, or the ▣ button above it) to save it — entities, models, scripts and all — into the project's `custom/` folder. It shows up in the **Prefabs** tab of the left panel (`⌥P`): click a card or drag it into the viewport to drop another copy in front of the camera. Copies are independent — editing one doesn't change the others. Rows placed from a prefab carry a ▣ marker, and the inspector says which prefab they came from. |
...
| **Game kit** | Under **Multiplayer Server** in the Prefabs tab there are five prefabs that make a round-based game without code. **Round Loop** is the clock the rest hang off: lobby, wave, break, wave — one countdown, the same for everybody. **Wave Director** sends the enemies, reading the numbers from the **Game Config** panel on the scene root, so you rebalance a fight by editing a table instead of a script. **Level Slots** swaps the arena between rounds. **Player Rig** gives every player a nameplate, a health bar and a gun. **Leaderboard** is a board on a panel that survives a restart. Each one carries a guide the AI assistant reads before writing anything that touches it. |
| **Spawnable prefabs** | A prefab your game makes copies of while it runs — the zombies, the pickups. Right-click its card and choose **Placement & spawning…**: turn **Spawnable** on, say how many may be alive at once, and choose whether it is spawned on demand or one per player. The same sheet says where the prefab sits in the scene — **Unplaced** (only copies, made while the game runs), **Editor & Play** (a copy is there from the start) or **Editing only** (you edit it in place and the running game never sees it). The card's chips say what the game guarantees about those copies: what the server owns, and what each player's own screen decides. |
| **Scene checks** | A card in the editor that reads what you wired together and says what cannot work — a wave asking for more enemies than the pool allows, the same number set in two places, an anchor you changed but never saved back to its prefab. The serious ones hold Play until they are fixed, with **Play anyway** if you disagree. |
| **Game Config** | A table of the numbers your game runs on — how many enemies in wave 5, how much damage a bite does — edited in the inspector on the scene root and written out as one file every script reads. Change a number and it lands at the next round, without a code edit. |
