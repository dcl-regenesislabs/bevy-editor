...
engine. Edit your scene in a live 3D viewport, write scripts with the built-in
assistant, and publish straight to your Decentraland world.
...
2. The scene opens paused in the editor: drag in models, move them with the
   ...
   tab next to it holds the prefabs you've saved for reuse — in this scene, in
...
The viewport is a live scene, paused. A few things are worth knowing:
...
| **Editing the code** | Saving from Script Studio rebuilds and restarts the scene for you. Code changed anywhere else — your own editor, the assistant — rebuilds too, and ▶ waits for that build and loads it before it runs, so you never have to restart by hand. |
...
| **Trigger Area** | A built-in prefab: an invisible area that knows who is standing in it. Place the **Trigger Area** card from the **Prefabs** tab — the area drops in front of the camera and its hierarchy row opens ready for you to type a name, and that name is what other scripts ask for. Then drag it into place and use the scale handles until it covers the doorway or the arena. Its settings say who it reacts to (just you, or anyone), how often an entry counts, and how long someone may step out before they count as gone. Other scripts react by asking for the area by name, so nothing has to be wired up; ask the assistant to "open this when someone's in Front Hall" and it writes that script for you. An area is decided on each player's own client, which is what you want for doors, lights and sound. |
| **Spawner** | A built-in prefab that makes copies of another prefab appear while the game runs — the enemies, the crates, the pickups. The quickest way in is the right-click menu: right-click anything in the scene and pick **Add a spawner**, and a spawner drops onto it already wired — to a Trigger Area it fires when a player walks in, to anything else when a player clicks it. Then pick what appears from its dropdown. The rest of its settings are the whole design: on a timer, or when another script asks for it by name; how many copies may exist at once and how long each one sticks around — several copies spread out on their own so they don't stack. Copies appear the instant the trigger fires, only on this player's client — no wiring, no code. |
| **Multiplayer Server prefabs** | Under **Multiplayer Server** in the Prefabs tab, four prefabs that build a round-based game with no code. **Game Flow** runs it from start to end — a lobby that waits for players, a countdown, rounds, and the winners between them; set the round length, or hand the ending to your own script and let the length be a ceiling. **Health & Respawn** gives players hit points the server owns and sends them back to a point you pick, with a "die below height" plane for anything you can fall off. **Leaderboard** is a panel that shows one of your game's boards — point it at the board your game writes and it paints the places; two panels on two boards, or two on one. **Announcer** shows a line on every player's client for a few seconds — the server says it once and it fades. Each one carries a guide the assistant reads before writing anything that touches it. To go past what they do on their own, [`docs/BUILD-A-MULTIPLAYER-GAME.md`](./docs/BUILD-A-MULTIPLAYER-GAME.md) builds one whole game — Tower of Madness — from these items plus nine files of your own. |
| **Spawning prefabs** | Every prefab can be spawned by your game — the zombies, the pickups. Build the thing in the scene, right-click it and pick **Create prefab…**: the dialog asks a name and when it appears — *From the start*, or *When spawned* (it moves to the Prefabs tab and the game brings copies in while playing). Pick the prefab in the **Spawner**'s dropdown and that is the whole setup. *From the start* and a Spawner are not the same thing: the folder holds the one you built, placed once; a Spawner brings in a fresh copy each time, capped and timed. The scene tree shows the split: **From the start** holds what players see the moment the game begins, **When spawned** holds what you built for the game to bring in while it plays — ordinary entities either way, edited in place, moved between the folders from the right-click menu. The card's chips say what the game guarantees about those copies: what the server owns, and what each player's own client decides. |
...
Ask for what the selected object should do and it writes a **Script component**,
