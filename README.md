...
| **Folders** | Keep the tree tidy: select things that belong together and press `⌘G` (or right-click → **Group into a folder**) — the selection is wrapped in a folder, ready for you to name. Drag rows in or out any time; everything keeps its place in the world. **Ungroup** (`⇧⌘G`) moves the contents up a level and removes the folder, and the folder button above the tree makes an empty one. Folders don't change how the game runs — though moving, hiding or locking one carries everything inside it, which is exactly what grabbing a whole room by its folder is for. |
