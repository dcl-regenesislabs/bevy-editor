...
| **Spawnable prefabs** | A prefab your game makes copies of while it runs — the zombies, the pickups. Build the thing in the scene, right-click it and pick **Create spawnable prefab…**: the dialog asks how many copies may be alive at once, whether they are made on demand or one per player, and whether the one you selected stays in the scene. Everything it asks can be changed later — the **⋯** button on the prefab's card (or a right-click) opens **Placement & spawning…**, which also says where the prefab sits in the scene: **Unplaced** (only the copies your game makes), **In the game** (a copy is there from the start) or **Editing only** (you edit it in place and the running game never sees it). The card's chips say what the game guarantees about those copies: what the server owns, and what each player's own screen decides. |
